...
void main() {
  ...
  group('IO', () {
    testWidgets('files created', (tester) async {
      ...
      expect(await File(LocalManager.themeFilePath!).exists(), true);
    });
    ...
    testWidgets('directories created', (tester) async {
      ...
      expect(
          await Directory(LocalManager.localBellDirectoryPath).exists(), true);
      expect(await Directory(LocalManager.reminderTextDirectoryPath).exists(),
          true);
      expect(
          await Directory(LocalManager.onStartApplicationDirectoryPath)
              .exists(),
          true);
    });
  });
  ...
  group('custom notification service initialized', () {
    ...
    testWidgets('notification sent', (tester) async {
      ...
      await AndroidBell.initServicesAsync();
      bool result =
          await bell.sendNotification('reminder', 'reminder', defaultReminder);
      expect(result, true);
    });
  });
}
